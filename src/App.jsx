...
const TEAM = [
  { name:"Aline Ventura", region:"Americas",
    ...
    median:{new:99, addon:84, update:42, all:80 }, mNote:{new:false,addon:false,update:false,all:false} },
  { name:"Daisy Marquez", region:"Americas",
    ...
    median:{new:200,addon:200,update:186,all:196}, mNote:{new:false,addon:false,update:true,all:false} },
  { name:"Jevon Jackson", region:"Americas",
    ...
    median:{new:58, addon:50, update:37, all:45 }, mNote:{new:false,addon:false,update:false,all:false} },
  { name:"Kristen Whitman", region:"Americas",
    ...
    median:{new:91, addon:90, update:39, all:83 }, mNote:{new:false,addon:false,update:false,all:false} },
  { name:"Kristina Quirouette", region:"Americas",
    ...
    median:{new:132,addon:72, update:76, all:88 }, mNote:{new:false,addon:false,update:false,all:false} },
  { name:"Mariana Freitas", region:"Americas",
    ...
    median:{new:79, addon:59, update:33, all:58 }, mNote:{new:false,addon:false,update:false,all:false} },
  { name:"Nia Gillenwater", region:"Americas",
    ...
    median:{new:83, addon:52, update:47, all:57 }, mNote:{new:false,addon:false,update:false,all:false} },
  { name:"Fabrizio Ramires", region:"Americas",
    ...
    median:{new:null,addon:null,update:38,all:38}, mNote:{new:true,addon:true,update:true,all:true} },
  { name:"Nathalie Lesmes Rodriguez", region:"EU",
    ...
    median:{new:74, addon:121,update:55, all:88 }, mNote:{new:false,addon:false,update:false,all:false} },
  { name:"Oleksii Kosenko", region:"EU",
    ...
    median:{new:176,addon:163,update:129,all:162}, mNote:{new:false,addon:false,update:false,all:false} },
  { name:"Prathiba Seetharaman", region:"EU",
    ...
    median:{new:null,addon:null,update:null,all:null}, mNote:{new:true,addon:true,update:true,all:true} },
];
...
function BacklogWoW({curr,prev}){
  ...
  // backlog growing = bad (red), shrinking = good (green)
...
export default function App(){
  ...
  const rows=useMemo(()=>filtered.map(m=>({
    ...
    _med:   m.median?.[reason]??null, _medN:m.mNote?.[reason]??false,
  ...
  return(
    <div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",padding:"20px 24px",background:"#fff",minHeight:"100vh"}}>
      ...
      <div style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap"}}>
        ...
        {card(`New cases${sfx}`,tN7,nWow>0?"#b91c1c":"#1a1a1a",`${THIS_WEEK} · vs ${tNp7} (${PREV_WEEK})`)}
        {card(`Closed${sfx}`,tCl7,cWow>=0?"#15803d":"#b91c1c",`${THIS_WEEK} · vs ${tCp7} (${PREV_WEEK})`)}
        {card(`Median close${sfx}`,tMed?`${tMed}d`:"—",tMed?tMed<70?"#15803d":tMed<100?"#92400e":"#b91c1c":null,"team median")}
      </div>
      ...
      <div style={{fontSize:11,color:"#aaa",marginTop:10}}>
        Week = Mon–Sun · This week: {THIS_WEEK} · Prior: {PREV_WEEK} · Open/aged = live · Backlog WoW = open now vs open last week (↑ bad) · Close WoW ↑ good · New WoW ↑ bad · * = &lt;10 closed cases
      </div>
    </div>
  );
}
